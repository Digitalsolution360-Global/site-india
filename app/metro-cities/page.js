...
export default function MetroCitiesPage() {
        ...
        async function fetchStats() {
            try {
                const categoriesList = [
                    { slug: 'gmb', dbCategory: 'Google Business' },
                    { slug: 'marketing', dbCategory: 'Digital Marketing' },
                    { slug: 'web', dbCategory: 'Web Development' },
                    { slug: 'content-writing', dbCategory: 'Content Writing' },
                    { slug: 'wordpress', dbCategory: 'Wordpress Development' },
                    { slug: 'social-media', dbCategory: 'Social Media' }
                ];
                const responses = await Promise.all(
                    categoriesList.map(async (cat) => {
                        const res = await fetch(`${API_BASE}/market/${cat.slug}/metrocities`);
                ...
                responses.forEach((json, idx) => {
                    const catName = categoriesList[idx].dbCategory;
                    if (json.success && json.data) {
                        categoryStats[catName] = json.data.length;
                        json.data.forEach(item => {
                            if (item.state_name) allStates.add(item.state_name);
                        });
                        totalCities += json.data.length; // Actually, total cities across categories, but let's just do unique if possible.
                    }
                });

                // to be more accurate, total metro cities usually is just for one category since they repeat
                const uniqueCitiesCount = responses[0]?.data?.length || 0; 

                setStats({
                    totalStates: allStates.size > 0 ? allStates.size : 20,
                    totalCities: uniqueCitiesCount > 0 ? uniqueCitiesCount : 150,
                    categoryStats
                });
            } catch (err) {
    ...
    return (
        <>
            ...
            <OGMetaTags serviceName="Digital Marketing Metro Cities" pageSlug="metro-cities" />
                        ...
                        <div className='flex flex-wrap gap-6'>
                            ...
                            <div className='flex items-center gap-3 bg-white/20 backdrop-blur-sm px-4 py-3 rounded-lg border border-white/30'>
                                ...
                                <div>
                                    ...
                                    <div className='text-sm text-gray-200'>Metro Cities</div>
                                </div>
                            </div>
                        </div>
                    </motion.div>
                    ...
                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8'>
                        {categories.map((category, index) => {
                            ...
                            return (
                                ...
                                >
                                    <Link href={`/metro-cities/${category.slug}`}>
